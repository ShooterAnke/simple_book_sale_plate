<template>
  <div>
    <div class="top">
      <el-select v-model="category_option" placeholder="请选择" @change="getCardData">
        <el-option
      ...
      <el-select v-model="search_option" placeholder="请选择">
        <el-option
          v-for="item in searchLabel"
          :key="item.value"
          :label="item.label"
          :value="item.value"
        >
      ...
      <el-input placeholder="输入查询" v-model="search"></el-input>
      <el-button type="primary" @click="getCardData">查询</el-button>
    </div>
      ...
      <el-col :span="span" v-for="(item, index) in cardData" :key="index">
        <el-card shadow="hover" class="card">
          ...
          <div class="order">
            ...
            <h1 v-if="item.isSend === 1&&item.isGet===0 " style="color: red">
              状态:待确认
            </h1>
            <h1 v-if="item.isSend === 0&&item.isGet===0 " style="color: red">
              状态:待发货
            </h1>
            <h1 v-if="item.isSend===1&&item.isGet===1" style="color: red">状态:已完成</h1>
          </div>
          <div class="bookOrder">
...
<script>
import { getCardDataByOption, confirmSend } from "@/api/orderApi.js";
  ...
  methods: {
    confirmHandler(item) {
      this.$confirm("确认发货？", "提示", {
        ...
        .then(() => {
          confirmSend(this.$store.state.user.token, item.id).then((res) => {
            if (res.data.code === 0) {
              this.$message({
    ...
    getCardData() {
      getCardDataByOption(
        ...
        if (res.data.code === 0) {
          console.log(res.data.data.cardData);
